 ...
 */
typedef int (*searcher_function)(const commodity comms[], int comms_count, comm_id_t ids[], int ids_count, int quantities[]);
...
/**
 * @brief Funkce, která provádí vyhledávání klíčů ids v poli comms pomocí BVS. Výsledky vyhledávání jsou zapisovány na odpovídající
 *        pozice do pole quantities.
 * @param comms Datové pole s komoditami, které bude prohledáváno.
 * @param comms_count Počet prvků pole comms.
 * @param ids Pole identifikátorů, které budou postupně vyhledávány v datovém poli comms.
...
/**
 * @brief Funkce pro porovnání dvou komodit. Definice této funkce je nutná pro funkce qsort a bsearch.
 * @param a První komodita.
 * @param b Druhá komodita.
 * @return int <0 pokud a je v pořadí před b, =0 pokud a je stejné jako b, >0 pokud a je v pořádí za b.
 */
int comm_compare(const void *a, const void *b);

/**
 * @brief Obdobné jako v minulém případě.
 * @param a Komodita.
 * @param b Klíč.
 * @return int <0 pokud a je v pořadí před b, =0 pokud a je stejné jako b, >0 pokud a je v pořádí za b.
 */
